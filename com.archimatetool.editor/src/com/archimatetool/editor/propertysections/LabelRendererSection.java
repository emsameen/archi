...
public class LabelRendererSection extends AbstractECorePropertySection {
    ...
    protected void createControls(Composite parent) {
        createLabel(parent, "Text Expression:", ITabbedLayoutConstants.STANDARD_LABEL_WIDTH, TextRenderer.MULTI_LINE ? SWT.NONE : SWT.CENTER);
        
        Control textControl;
        
        if(TextRenderer.MULTI_LINE) {
        ...
        fTextRender = new PropertySectionTextControl(textControl, TextRenderer.FEATURE_NAME) {
            @Override
            protected void textChanged(String oldText, String newText) {
                ...
                for(EObject eObject : getEObjects()) {
                    if(isAlive(eObject)) {
                        Command cmd = new FeatureCommand("Set expression", (IFeatures)eObject, TextRenderer.FEATURE_NAME, newText, ""); //$NON-NLS-2$
                        if(cmd.canExecute()) {
                            result.add(cmd);
                        }
                    }
